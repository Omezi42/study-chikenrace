...
*   **ビジュアル ＆ UI**:
    ...
    *   **ランキング導線**: 全世界のプレイヤーのスコアを確認できるランキングボタンなどを配置し、対戦へのモチベーションを高める。
    *   **背景装飾**: 机の上を表現するため、文房具アイテム（消しゴム、鉛筆、ノート）やカードのイラストが少し傾いて散らばっている。デイリー目標などの複雑なUIは排除し、クリーンな画面を保つ。
*   **UX ＆ アニメーション**:
...
*   **ビジュアル ＆ UI**:
    ...
    *   **アイテム詳細表示**: 編成中、各アイテムの詳細な効果やステータスを即座に確認できるUIを備え、戦略的なデッキ構築をサポートする。
*   **UX ＆ アニメーション**:
    *   スロットのボタンを押すと「X番枠に置くアイテムを選択」のダイアログオーバーレイがフェードイン。解放済みのアイテムから1つ選択可能で、詳細効果を確認しながらスムーズに入れ替えを行える。
...
*   **UXインタラクション**:
    ...
    *   **手札ファン（扇形）配置と自動縮小**:
        *   引いたカードは中央に向かって扇形に広がるように、角度（-12度〜12度の範囲）とY座標が動的算出（`y = -abs(offset_x) * 0.08`）されて配置され、手札らしさを演出。ドローされた瞬間から右ページの適正な定位置に表示される。
...
*   **UIレイアウト**:
    *   スマートフォン「チキスタ」のタイムラインタブが開かれ、画面中央など見やすい位置に適切に配置される（左下への偏りを解消し、視認性を向上）。右側には「明日の勉強へ進む」ボタンが配置される。
...
*   **UXインタラクション**:
    *   **固定ドロー体験**: チュートリアル中のカードドローは完全に固定されており、プレイヤーが「バースト」や「カウンティング」といったゲームのコアメカニクスを確実に体験できるように設計されている。
    *   状況の変化（バースト発生時や確率計算時など）に合わせて、付箋テキストが動的に更新され、プレイヤーに思考のヒントを与える。
