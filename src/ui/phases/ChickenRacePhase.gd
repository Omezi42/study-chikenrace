...
func _on_setup(setup_data: Dictionary) -> void:
	...
	standing_phone.position = Vector2(-260, 150) # Leave 40px on screen when closed
	...
	phone_toggle_btn.position = Vector2(260, 180) # Sits outside the phone body container (starts at x=260)
	...
	opt_btn.position = Vector2(1330, 20)
...
func perform_animated_draw(card: Dictionary, on_complete: Callable = Callable()) -> void:
	...
	var card_vbox = card_ui.get_child(0)
	if card_vbox:
...
func create_card_visual(card: Dictionary) -> Button:
	var card_ui = Button.new()
	card_ui.custom_minimum_size = Vector2(160, 220)
	card_ui.pivot_offset = Vector2(80, 110)
	
	var item_info = CardData.ITEMS.get(card["item_id"], {"role": CardData.ROLE_PREP, "name": card.get("name", "アイテム")})
	var card_style = StyleBoxFlat.new()
	card_style.bg_color = DeskTheme.COLOR_CRAFT
	card_style.border_color = CardData.get_role_color(item_info["role"])
	...
	card_ui.add_theme_stylebox_override("normal", card_style)
	card_ui.add_theme_stylebox_override("hover", card_style)
	card_ui.add_theme_stylebox_override("pressed", card_style)
	card_ui.add_theme_stylebox_override("focus", card_style)
	
	var card_vbox = VBoxContainer.new()
	card_vbox.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	card_vbox.alignment = BoxContainer.ALIGNMENT_CENTER
	card_vbox.add_theme_constant_override("separation", 6)
	card_ui.add_child(card_vbox)
	...
	var sub_icon_path = CardData.get_subject_icon_path(card["subject"])
	if sub_icon_path != "":
		...
		card_ui.add_child(subject_badge)
	
	# Value
	var val_label = Label.new()
	val_label.text = str(card["value"])
	...
	if img_path != "":
		...
		# Small spacer for fallback spacing consistent with premium visual feel
	...
	name_lbl.text = item_info["name"] if item_info else "カード"
	...
	effect_lbl.add_theme_color_override("font_color", Color("ff4081") if item_info and item_info["role"] == CardData.ROLE_PUSH else Color(DeskTheme.COLOR_INK, 0.6))
...
func activate_item_effect(card: Dictionary) -> void:
	...
	match item_id:
		"item_sticky_note":
			...
			DeskTheme.show_toast(self, "付箋の効果！次のドローは【%s】が出やすくなった！" % sub_jp)
			
		"item_eraser":
			deck.eraser_charges += 1
			DeskTheme.show_toast(self, "消しゴムの効果！眠気回避（被り無効化）をチャージ！")
			
		"item_ruler":
			var peeked = deck.peek_cards(1)
			if peeked.size() > 0:
				show_peek_sticky(peeked)
				DeskTheme.show_toast(self, "定規の効果！山札の次の一枚をのぞき見した！")
				
		"item_wordbook":
			var peeked = deck.peek_cards(3)
			if peeked.size() > 0:
				show_peek_sticky(peeked)
				DeskTheme.show_toast(self, "単語帳の効果！山札の次の三枚をのぞき見した！")
				
		"item_mech_pencil":
			deck.next_draw_bonus_points = 2
			DeskTheme.show_toast(self, "シャーペンの効果！次に引く2枚の得点＋3点！")
			
		"item_memo_cards":
			start_card_selection("memo_cards", "【暗記カード】入れ替える手札のカードを選んでください。")
				
		"item_highlighter":
			deck.highlighter_active = true
			DeskTheme.show_toast(self, "蛍光ペンの効果！この時限のコンボボーナスが1.5倍！")
			
		"item_blue_pen":
			deck.blue_pen_active = true
			DeskTheme.show_toast(self, "青ペンの効果！場に出ている国語・英語の得点が1.5倍！")
			
		"item_cushion":
			DeskTheme.show_toast(self, "座布団の効果！ダウト失敗時のペナルティ失点が半減！")
			
		"item_memo_app":
			var card1 = deck.draw_card()
			var card2 = deck.draw_card()
			var msg = "メモアプリの効果！カードを2枚引いた！"
			DeskTheme.show_toast(self, msg)
			
			draw_btn.disabled = true
			stop_btn.disabled = true
			
			if not card1.is_empty():
				perform_animated_draw(card1, func():
					if not card2.is_empty():
						perform_animated_draw(card2, func():
							start_card_selection("memo_app", "【メモアプリ】捨てるカードを1枚選んでください。")
						)
					else:
						start_card_selection("memo_app", "【メモアプリ】捨てるカードを1枚選んでください。")
				)
		...
		"item_cheat_sheet":
			DeskTheme.show_toast(self, "ズルいカンペの効果！本日のブラフ上限が＋16点！")
			
		"item_compass":
			var count = deck.activate_compass()
			DeskTheme.show_toast(self, "コンパスの効果！被り候補カードが山札に %d 枚残っています！" % count)
		...
		"item_energy_drink":
			...
			if burst_chance > 0 and randf() < burst_chance:
				...
				DeskTheme.show_toast(self, "エナジードリンクの効果！この時限の獲得点数2倍！")
				
		"item_red_sheet":
			deck.red_sheet_active = true
			DeskTheme.show_toast(self, "赤シートの効果！バースト札を一度だけ自動で捨てる！")
			
		"item_thick_book":
			deck.activate_thick_book()
			DeskTheme.show_toast(self, "分厚い参考書の効果！高得点(+15点)を山札に3枚追加！")
			
		"item_amulet":
			DeskTheme.show_toast(self, "お守りの効果！寝落ち（バースト）しても点数の50%をキープ！")
			
		"item_night_note":
			deck.activate_night_note()
			session.max_hours_today = 4
			DeskTheme.show_toast(self, "追込みノートの効果！本日の時限が4時限に増加した！")
			
		"item_copy_answer":
			DeskTheme.show_toast(self, "解答写しの効果！ブラフ上限＋25点（嘘バレペナルティ2倍）！")
			
		"item_timer":
			DeskTheme.show_toast(self, "タイマーの効果！正確なバースト確率が常時表示された！")
			
		"item_study_chat":
			DeskTheme.show_toast(self, "勉強会チャットの効果！ダウト成功点＋6点！")
		...
		"item_expected_questions":
			...
			match sub:
				...
				CardData.SUBJECT_SCIENCE: sub_jp = "理科"
				CardData.SUBJECT_SOCIAL: sub_jp = "社会"
			DeskTheme.show_toast(self, "予想問題集の効果！次の3枚が【%s】で固定された！" % sub_jp)
		...
		"item_cafe_latte":
			...
			if not latte_card.is_empty():
				DeskTheme.show_toast(self, "カフェラテの効果！安全に【%s (%d点)】を引いた！" % [latte_card["name"], latte_card["value"]])
				perform_animated_draw(latte_card, func():
					update_ui()
					draw_btn.disabled = false
					stop_btn.disabled = false
				)
		...
		"item_earplugs":
			DeskTheme.show_toast(self, "耳栓の効果！ダウト失敗時のペナルティを10点軽減！")
			
		"item_cram_school_print":
			DeskTheme.show_toast(self, "塾プリントの効果！5教科コンボのワイルドカードとして機能！")
			
		"item_forget_notebook":
			var val = deck.activate_forget_notebook()
			if val > 0:
				DeskTheme.show_toast(self, "忘却のノートの効果！不要なカード（%d点）をデッキから削除！" % val)
			else:
				DeskTheme.show_toast(self, "忘却のノートの効果！不要なカードを削除した！")
...
func show_peek_sticky(peeked: Array) -> void:
	...
	active_peek_sticky.position = Vector2(1180, 520)
...
func arrange_hand_fan() -> void:
	...
	for idx in range(count):
		...
		child.position = Vector2(center_x + offset_x - (child.custom_minimum_size.x * scale_mult) / 2.0, base_y + offset_y)
	...
	if yesterday_day < 1:
		for key in scores.keys():
			...
			if key == "player" and Global.player_name != "":
				...
				name = AIManager.CPU_OPPONENTS[key]["name"]
	...
	for day_idx in range(1, yesterday_day + 1):
		...
		for p_id in scores.keys():
			...
			if is_liar and final_exposed:
				...
				var deck_config = Global.current_deck if p_id == "player" else AIManager.CPU_OPPONENTS[p_id]["deck"]
...
func _on_card_ui_mouse_entered(card: Dictionary, card_ui: Button) -> void:
	...
	tween.tween_property(card_ui, "scale", card_ui.scale * scale_mult, 0.15)
	
	var lift_y = -30 if is_selecting_card else -20
	tween.tween_property(card_ui, "position:y", card_ui.position.y + lift_y, 0.15)
...
func _on_card_ui_mouse_exited(card_ui: Button) -> void:
	...
	# arrange_hand_fan restores correct positioning and scale
	arrange_hand_fan()
