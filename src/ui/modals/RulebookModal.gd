...
var _last_is_port = false

func _ready() -> void:
	if has_node("/root/ResponsiveScaler"):
		_last_is_port = get_node("/root/ResponsiveScaler").is_portrait()
	layer = 102
	...
	# Footer Navigation
	var footer_container = VBoxContainer.new() if is_port else HBoxContainer.new()
	footer_container.alignment = BoxContainer.ALIGNMENT_CENTER
	footer_container.add_theme_constant_override("separation", 10 if is_port else 30)
	vbox.add_child(footer_container)
	
	var nav_hbox = HBoxContainer.new()
	nav_hbox.alignment = BoxContainer.ALIGNMENT_CENTER
	nav_hbox.add_theme_constant_override("separation", 10 if is_port else 30)
	footer_container.add_child(nav_hbox)
	
	prev_btn = Button.new()
	prev_btn.text = " ＜" if is_port else " ＜ 前へ "
	prev_btn.custom_minimum_size = Vector2(60, 50) if is_port else Vector2(140, 50)
	prev_btn.add_theme_font_override("font", DeskTheme.get_font())
	prev_btn.add_theme_font_size_override("font_size", DeskTheme.scaled_font(20))
	...
	page_dots_hbox.add_theme_constant_override("separation", 8 if is_port else 15)
	nav_hbox.add_child(page_dots_hbox)
	...
	next_btn = Button.new()
	next_btn.text = "＞ " if is_port else " 次へ ＞ "
	next_btn.custom_minimum_size = Vector2(60, 50) if is_port else Vector2(140, 50)
	next_btn.add_theme_font_override("font", DeskTheme.get_font())
	next_btn.add_theme_font_size_override("font_size", DeskTheme.scaled_font(20))
	...
	play_tutorial_btn.text = " ★ チュートリアル ★ " if is_port else " ★ チュートリアル対戦へ ★ "
	play_tutorial_btn.custom_minimum_size = Vector2(250, 50) if is_port else Vector2(300, 50)
	play_tutorial_btn.add_theme_font_override("font", DeskTheme.get_font())
	play_tutorial_btn.add_theme_font_size_override("font_size", DeskTheme.scaled_font(18))
	_apply_tutorial_button_style(play_tutorial_btn)
	play_tutorial_btn.pressed.connect(_start_tutorial)
	if is_port:
		footer_container.add_child(play_tutorial_btn)
	else:
		nav_hbox.add_child(play_tutorial_btn)
	...
	tween.tween_property(modal, "scale", Vector2.ONE * target_s, 0.3)
	
	_show_page(current_page, true)
...
func _get_target_scale() -> float:
	...
	var base_size = _get_base_size()
	var avail_w = max(vp_size.x - 20.0, 100.0)
	var avail_h = max(vp_size.y - 20.0, 100.0)
	var s = min(avail_w / base_size.x, avail_h / base_size.y)
	return clamp(s, 0.3, 3.0)

func _on_scale_changed(_new_scale: float) -> void:
	var current_port = false
	if has_node("/root/ResponsiveScaler"):
		current_port = get_node("/root/ResponsiveScaler").is_portrait()
	if current_port != _last_is_port:
		var parent = get_parent()
		if parent:
			var new_modal = load("res://src/ui/modals/RulebookModal.gd").new()
			new_modal.current_page = self.current_page
			parent.add_child(new_modal)
			self.queue_free()
		return
	_update_responsive_layout()
...
func _update_responsive_layout() -> void:
	...
	var actual_size = modal.get_combined_minimum_size()
	modal.pivot_offset = actual_size * 0.5
	if modal.scale != Vector2.ZERO:
		modal.scale = Vector2.ONE * s
	modal.position = (vp_size - actual_size) * 0.5
...
# --- Page 0: Intro ---
func _setup_page_0(page: Control, visual_area: Control, rtb: RichTextLabel, title_lbl: Label, is_port: bool) -> void:
	title_lbl.text = "① テストまであと3日！"
	...
	notebook.custom_minimum_size = Vector2(340, 200) if is_port else Vector2(400, 250)
	notebook.position = Vector2(46, 120) if is_port else Vector2(290, 30)
	...
	c1.scale = Vector2(0.65, 0.65) if is_port else Vector2(0.8, 0.8)
	c1.position = Vector2(80, 150) if is_port else Vector2(330, 60)
	c1.rotation_degrees = -5
	ctrl.add_child(c1)
	
	var c2 = CardVisual.create({"value": 10})
	c2.scale = Vector2(0.65, 0.65) if is_port else Vector2(0.8, 0.8)
	c2.position = Vector2(160, 150) if is_port else Vector2(450, 60)
	c2.rotation_degrees = 5
	ctrl.add_child(c2)
	
	var c3 = CardVisual.create({"value": 7})
	c3.scale = Vector2(0.65, 0.65) if is_port else Vector2(0.8, 0.8)
	c3.position = Vector2(240, 150) if is_port else Vector2(570, 60)
	c3.rotation_degrees = 15
	ctrl.add_child(c3)
	
	# ライバルの嘘申告を演出する吹き出し
	var rival_bubble = PanelContainer.new()
	rival_bubble.position = Vector2(20, 20) if is_port else Vector2(50, 50)
	...
	bubble_name.add_theme_font_size_override("font_size", DeskTheme.scaled_font(13))
	...
	bubble_declared.add_theme_font_size_override("font_size", DeskTheme.scaled_font(20))
	...
	bubble_actual.add_theme_font_size_override("font_size", DeskTheme.scaled_font(13))
	bubble_actual.add_theme_color_override("font_color", Color(DeskTheme.COLOR_INK, 0.45))
	bubble_vbox.add_child(bubble_actual)
	
	rtb.text = "自習で稼ぐ[b]実点（ホントの点数）[/b]と、SNSに投稿する[b]申告点（嘘でもOK）[/b]を設定します。\n3日間の騙し合いの末、最終答え合わせで[color=#ff4081][b]最も得点が高いプレイヤー[/b][/color]が優勝です！"
	
	page.set_meta("play_animations", func():
		...
		# Cards bob
		tw.tween_property(c1, "position:y", 50.0, 1.0).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
		tw.parallel().tween_property(c2, "position:y", 70.0, 1.0).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
		tw.parallel().tween_property(c3, "position:y", 50.0, 1.0).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
	
		tw.tween_property(c1, "position:y", 60.0, 1.0).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
		tw.parallel().tween_property(c2, "position:y", 60.0, 1.0).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
		tw.parallel().tween_property(c3, "position:y", 60.0, 1.0).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
	...
	_disable_mouse_interactions(visual_area)
...
func _build_page(idx: int) -> Control:
	...
	# Title container and label (Unified layout to avoid overlap)
	...
	rtb.scroll_active = false
	rtb.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	...
	match idx:
		0: _setup_page_0(page, visual_area, rtb, title_lbl, is_port) # Intro
		1: _setup_page_1(page, visual_area, rtb, title_lbl, is_port) # Draw
		2: _setup_page_2(page, visual_area, rtb, title_lbl, is_port) # Burst
		3: _setup_page_3(page, visual_area, rtb, title_lbl, is_port) # Report
		4: _setup_page_4(page, visual_area, rtb, title_lbl, is_port) # Doubt
		5: _setup_page_5(page, visual_area, rtb, title_lbl, is_port) # Score calculation
...
func _setup_page_1(page: Control, visual_area: Control, rtb: RichTextLabel, title_lbl: Label, is_port: bool) -> void:
	title_lbl.text = "② 自習で実点を積む"
	title_lbl.add_theme_color_override("font_color", DeskTheme.COLOR_GREEN)
	...
	var notebook = PanelContainer.new()
	notebook.custom_minimum_size = Vector2(340, 200) if is_port else Vector2(400, 250)
	notebook.position = Vector2(46, 120) if is_port else Vector2(480, 20)
	notebook.add_theme_stylebox_override("panel", DeskTheme.create_craft_panel())
	DeskTheme.add_ruled_lines(notebook)
	ctrl.add_child(notebook)
	
	var deck_pos = Vector2(280, 20) if is_port else Vector2(100, 40)
	var deck = _create_card_back()
	deck.position = deck_pos
	deck.scale = Vector2(0.65, 0.65) if is_port else Vector2(0.8, 0.8)
	ctrl.add_child(deck)
	
	var deck_lbl = Label.new()
	deck_lbl.text = "山札"
	deck_lbl.add_theme_font_override("font", DeskTheme.get_font())
	deck_lbl.add_theme_font_size_override("font_size", DeskTheme.scaled_font(20))
	deck_lbl.add_theme_color_override("font_color", DeskTheme.COLOR_INK)
	deck_lbl.position = Vector2(160, 120) if is_port else Vector2(130, 230)
	ctrl.add_child(deck_lbl)
	
	var c1 = CardVisual.create({"value": 4})
	c1.scale = Vector2(0.65, 0.65) if is_port else Vector2(0.8, 0.8)
	c1.position = Vector2(80, 150) if is_port else Vector2(520, 60)
	c1.rotation_degrees = -5
	ctrl.add_child(c1)
	
	var c2 = CardVisual.create({"value": 10})
	c2.scale = Vector2(0.65, 0.65) if is_port else Vector2(0.8, 0.8)
	c2.position = Vector2(160, 150) if is_port else Vector2(640, 60)
	c2.rotation_degrees = 5
	ctrl.add_child(c2)
	...
	var c3_back = _create_card_back()
	c3_back.scale = Vector2(0.65, 0.65) if is_port else Vector2(0.8, 0.8)
	c3_back.position = deck_pos
	c3_back.modulate.a = 0
	ctrl.add_child(c3_back)
	
	rtb.text = "自習フェーズで山札からカードを引き、カードの[color=#ff4081][b]数字の合計[/b][/color]がその日の「実点」になります。\n[color=#888888]※山札（全55枚）には数字 N（1〜10）のカードがそれぞれ N 枚入っています（10は10枚、1は1枚）。[/color]"
	...
	page.set_meta("play_animations", func():
		...
		var target_pos = Vector2(240, 150) if is_port else Vector2(760, 60)
		tw.tween_property(c3_back, "modulate:a", 1.0, 0.1)
		tw.parallel().tween_property(c3_back, "position", target_pos, 0.6).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_OUT)
...
func _setup_page_2(page: Control, visual_area: Control, rtb: RichTextLabel, title_lbl: Label, is_port: bool) -> void:
	title_lbl.text = "③ 欲張るな危険！「バースト」"
	title_lbl.add_theme_color_override("font_color", Color("#ff4081"))
	...
	var notebook = PanelContainer.new()
	notebook.custom_minimum_size = Vector2(340, 200) if is_port else Vector2(500, 260)
	var nb_pos = Vector2(46, 140) if is_port else Vector2(240, 30)
	notebook.position = nb_pos
	notebook.add_theme_stylebox_override("panel", DeskTheme.create_craft_panel())
	DeskTheme.add_ruled_lines(notebook)
	ctrl.add_child(notebook)
	
	var c7_1 = CardVisual.create({"value": 7})
	c7_1.scale = Vector2(0.65, 0.65) if is_port else Vector2(0.8, 0.8)
	c7_1.position = Vector2(100, 150) if is_port else Vector2(300, 50)
	c7_1.rotation_degrees = -5
	ctrl.add_child(c7_1)
	
	var deck = _create_card_back()
	var deck_pos = Vector2(280, 20) if is_port else Vector2(50, 40)
	deck.position = deck_pos
	deck.scale = Vector2(0.65, 0.65) if is_port else Vector2(0.8, 0.8)
	ctrl.add_child(deck)
	
	var c7_2 = CardVisual.create({"value": 7})
	c7_2.scale = Vector2(0.65, 0.65) if is_port else Vector2(0.8, 0.8)
	c7_2.position = deck_pos
	c7_2.modulate.a = 0
	ctrl.add_child(c7_2)
	
	var c7_2_back = _create_card_back()
	c7_2_back.scale = Vector2(0.65, 0.65) if is_port else Vector2(0.8, 0.8)
	c7_2_back.position = deck_pos
	...
	burst_lbl.add_theme_font_size_override("font_size", 48 if is_port else 54)
	...
	burst_lbl.position = Vector2(110, 180) if is_port else Vector2(350, 80)
	...
	prob_panel.position = Vector2(20, 10) if is_port else Vector2(640, 20)
	...
	prob_title_lbl.add_theme_font_size_override("font_size", DeskTheme.scaled_font(13))
	...
	prob_pct_lbl.add_theme_font_size_override("font_size", DeskTheme.scaled_font(16))
	prob_pct_lbl.add_theme_color_override("font_color", DeskTheme.COLOR_INK)
	prob_vbox.add_child(prob_pct_lbl)
	rtb.text = "すでに手札にあるカードと[color=#ff4081][b]同じ数字を引く[/b][/color]と「[color=#ff4081][b]寝落ち（バースト）[/b][/color]」となり、点数が0点になります。\n画面に表示されるバースト確率を見ながら、欲張らずに「休憩（確定）」しましょう。"
	...
	page.set_meta("play_animations", func():
		...
		tw.tween_callback(func():
			...
			c7_2.scale = Vector2(0.0, 0.65 if is_port else 0.8)
		...
		# カードを引く → 確率が上昇しオレンジに
		var target_pos = Vector2(180, 150) if is_port else Vector2(460, 50)
		tw.tween_property(c7_2_back, "modulate:a", 1.0, 0.1)
		tw.parallel().tween_property(c7_2_back, "position", target_pos, 0.5).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_OUT)
		...
		tw.tween_callback(func():
			...
			c7_2.rotation_degrees = c7_2_back.rotation_degrees
			c7_2.modulate = Color.WHITE
			c7_2_back.modulate = Color.TRANSPARENT
		)
		tw.tween_property(c7_2, "scale:x", 0.65 if is_port else 0.8, 0.2).set_ease(Tween.EASE_OUT)
	
		# バースト！
		tw.tween_property(burst_lbl, "scale", Vector2.ONE, 0.2).set_trans(Tween.TRANS_BACK)
		tw.parallel().tween_property(c7_1, "modulate:a", 0.0, 0.2)
		tw.parallel().tween_property(c7_2, "modulate:a", 0.0, 0.2)
		tw.tween_interval(1.5)
		return [tw]
	)
...
func _setup_page_3(page: Control, visual_area: Control, rtb: RichTextLabel, title_lbl: Label, is_port: bool) -> void:
	title_lbl.text = "④ SNS報告は戦場。「嘘」も戦略！"
	...
	phone_wrapper.position = Vector2(216, 210) if is_port else Vector2(490, 150)
	phone_wrapper.scale = Vector2(0.35, 0.35) if is_port else Vector2(0.32, 0.32)
	...
	phone_style.border_width_top = 12
	phone_style.border_width_bottom = 12
	...
	screen_container.custom_minimum_size = Vector2(396, 816) # Force minimum size to prevent collapsing
	...
	screen_bg.corner_radius_top_left = 28
	screen_bg.corner_radius_top_right = 28
	screen_bg.corner_radius_bottom_left = 28
	screen_bg.corner_radius_bottom_right = 28
	...
	time_lbl.text = "16:00"
	time_lbl.add_theme_font_size_override("font_size", DeskTheme.scaled_font(14))
	...
	app_name.text = "チキスタ"
	app_name.add_theme_font_size_override("font_size", DeskTheme.scaled_font(14))
	...
	decl_title.add_theme_font_size_override("font_size", DeskTheme.scaled_font(16))
	...
	declared_score_label.add_theme_font_size_override("font_size", DeskTheme.scaled_font(80))
	...
	actual_val.add_theme_font_size_override("font_size", DeskTheme.scaled_font(16))
	...
	# Generate white grabber icon dynamically
	...
	emote_title.add_theme_font_size_override("font_size", DeskTheme.scaled_font(14))
	...
	for e in emotes:
		...
		btn.add_theme_font_size_override("font_size", DeskTheme.scaled_font(14))
	...
	warning_text.add_theme_font_size_override("font_size", DeskTheme.scaled_font(13))
	...
	submit_btn.add_theme_font_size_override("font_size", DeskTheme.scaled_font(18))
	...
	report_slider.value_changed.connect(func(v):
		...
		if v > 20:
			declared_score_label.add_theme_color_override("font_color", Color("#ff3d00"))
	...
	rtb.text = "1日の終わりに、実点より高い点数を[color=#d500f9][b]「嘘（ブラフ）の点数」[/b][/color]としてSNSに投稿できます。\nライバルを焦らせることができますが、嘘が暴かれたときは失点するリスクがあります。" # = "[center][b]◆ チキスタ投稿と「嘘（ブラフ）」の戦略 ◆[/b][/center]\n1日の終わりに、その日の勉強成果をSNS『チキスタ』に投稿（報告）します。\nスライダーを操作して、実際の実点より高い[color=#d500f9][b]「嘘（ブラフ）の点数」[/b][/color]を申告し、ライバルを焦らせることができます。\nただし、嘘があまりにも高すぎると、相手に見破られたとき（ダウト）に大損します。"
	
	var cursor = _create_cursor()
	cursor.position = Vector2(220, 320) if is_port else Vector2(600, 300)
	cursor.modulate.a = 0
	...
	page.set_meta("play_animations", func():
		...
		var get_target_pos = func(node: Control):
			return ctrl.get_global_transform().affine_inverse() * (node.get_global_transform() * (node.size * 0.5))
			
		var start_drag_pos = func():
			return ctrl.get_global_transform().affine_inverse() * (report_slider.get_global_transform() * Vector2(20, report_slider.size.y * 0.5))
		var end_drag_pos = func():
			return ctrl.get_global_transform().affine_inverse() * (report_slider.get_global_transform() * Vector2(report_slider.size.x * 0.8, report_slider.size.y * 0.5))
		...
		tw.tween_callback(func(): 
			...
			cursor.position = start_drag_pos.call()
			cursor.modulate.a = 0
		...
		tw.tween_method(func(pos): cursor.position = pos, start_drag_pos.call(), start_drag_pos.call(), 0.1)
		tw.tween_property(cursor, "position", start_drag_pos.call(), 0.4).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
		...
		tw.parallel().tween_property(cursor, "position", end_drag_pos.call(), 1.2).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
		
		# Release
		tw.tween_property(cursor, "scale", Vector2.ONE, 0.1)
		tw.tween_interval(0.3)
	
		# Move to "confident" emote button
		var confident_btn: Button = null
		for item in emote_buttons:
			if item["key"] == "confident":
				confident_btn = item["btn"]
				
		var get_confident_pos = func():
			return get_target_pos.call(confident_btn)
			
		tw.tween_property(cursor, "position", get_confident_pos.call(), 0.5).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
		tw.tween_property(cursor, "scale", Vector2(0.8, 0.8), 0.1)
		tw.tween_callback(func(): 
			for item in emote_buttons:
				var btn = item["btn"]
		...
		var get_submit_pos = func():
			return get_target_pos.call(submit_btn)
			
		tw.tween_property(cursor, "position", get_submit_pos.call(), 0.5).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
...
func _setup_page_4(page: Control, visual_area: Control, rtb: RichTextLabel, title_lbl: Label, is_port: bool) -> void:
	title_lbl.text = "⑤ チキスタでの「ダウト」宣言"
	title_lbl.add_theme_color_override("font_color", Color("#d50000"))
	...
	post_panel.custom_minimum_size = Vector2(360, 120) if is_port else Vector2(400, 120)
	post_panel.position = Vector2(36, 60) if is_port else Vector2(290, 40)
	...
	pp_name.add_theme_font_size_override("font_size", DeskTheme.scaled_font(16))
	...
	pp_score.add_theme_font_size_override("font_size", DeskTheme.scaled_font(24))
	...
	doubt_btn.position = Vector2(116, 205) if is_port else Vector2(390, 185)
	doubt_btn.add_theme_font_override("font", DeskTheme.get_font())
	doubt_btn.add_theme_font_size_override("font_size", DeskTheme.scaled_font(40))
	...
	result_lbl.text = "ダウト成功！"
	result_lbl.add_theme_font_override("font", DeskTheme.get_font())
	result_lbl.add_theme_font_size_override("font_size", DeskTheme.scaled_font(36))
	result_lbl.add_theme_color_override("font_color", Color("#e53935"))
	...
	result_lbl.position = Vector2(20, 100) if is_port else Vector2(290, 80)
	result_lbl.scale = Vector2.ZERO
	result_lbl.pivot_offset = Vector2(200, 20)
	ctrl.add_child(result_lbl)
	
	rtb.text = "他プレイヤーの投稿とログを見比べ、嘘を疑う相手に「[color=#d50000][b]ダウト[/b][/color]」を宣言できます。\nダウトは1日最大3回まで宣言可能です。詳細ログから相手のドロー枚数を確認しましょう。"
	
	var cursor = _create_cursor()
	cursor.position = Vector2(310, 320) if is_port else Vector2(580, 300)
	cursor.modulate.a = 0
	ctrl.add_child(cursor)
	...
	page.set_meta("play_animations", func():
		...
		tw.tween_callback(func():
			...
			cursor.position = ctrl.get_global_transform().affine_inverse() * (doubt_btn.get_global_transform() * Vector2(doubt_btn.size.x * 0.8, doubt_btn.size.y * 1.5))
			cursor.modulate.a = 0
			cursor.scale = Vector2.ONE
		)
		tw.tween_interval(0.5)
		...
		# カーソル移動
		tw.tween_property(cursor, "modulate:a", 1.0, 0.2)
		tw.tween_property(cursor, "position", get_target_pos.call(doubt_btn), 0.5).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
		...
		tw.parallel().tween_property(post_panel, "modulate:a", 0.3, 0.2)
...
# --- Page 5: Score Calculation (Blackboard animation) ---
func _setup_page_5(page: Control, visual_area: Control, rtb: RichTextLabel, title_lbl: Label, is_port: bool) -> void:
	title_lbl.text = "⑥ 最終答え合わせと得点計算"
	title_lbl.add_theme_color_override("font_color", Color("#1b5e20"))
	...
	board_panel.custom_minimum_size = Vector2(360, 220) if is_port else Vector2(400, 250)
	board_panel.position = Vector2(36, 80) if is_port else Vector2(290, 30)
	var board_style = StyleBoxFlat.new()
	board_style.bg_color = Color("#1b5e20")
	board_style.border_color = Color("#8d6e63")
	board_style.border_width_left = 8
	board_style.border_width_right = 8
	board_style.border_width_top = 8
	board_style.border_width_bottom = 8
	...
	board_style.content_margin_left = 20
	board_style.content_margin_right = 20
	board_style.content_margin_top = 15
	board_style.content_margin_bottom = 15
	...
	board_title_lbl.text = "成績発表 (答え合わせ)"
	...
	board_title_lbl.add_theme_font_size_override("font_size", DeskTheme.scaled_font(20))
	...
	board_sep.color = Color(1, 1, 1, 0.4)
	board_sep.custom_minimum_size = Vector2(0, 2)
	board_vbox.add_child(board_sep)
	
	var detail_vbox = VBoxContainer.new()
	detail_vbox.add_theme_constant_override("separation", 6)
	board_vbox.add_child(detail_vbox)
	
	# Score lines for dynamic animation
	var line_actual = Label.new()
	line_actual.text = "① 実際の実点：30点"
	line_actual.add_theme_font_override("font", DeskTheme.get_font())
	line_actual.add_theme_font_size_override("font_size", DeskTheme.scaled_font(16))
	line_actual.add_theme_color_override("font_color", Color.WHITE)
	line_actual.modulate.a = 0
	detail_vbox.add_child(line_actual)
	
	var line_declared = Label.new()
	line_declared.text = "② SNS申告点：65点 (盛り幅 +35点)"
	line_declared.add_theme_font_override("font", DeskTheme.get_font())
	line_declared.add_theme_font_size_override("font_size", DeskTheme.scaled_font(16))
	line_declared.add_theme_color_override("font_color", Color.WHITE)
	line_declared.modulate.a = 0
	detail_vbox.add_child(line_declared)
	
	var line_result = Label.new()
	line_result.text = "③ ダウト被弾！嘘バレ発生！"
	line_result.add_theme_font_override("font", DeskTheme.get_font())
	line_result.add_theme_font_size_override("font_size", DeskTheme.scaled_font(16))
	line_result.add_theme_color_override("font_color", Color("#ff4444"))
	line_result.modulate.a = 0
	detail_vbox.add_child(line_result)
	
	var line_penalty = Label.new()
	line_penalty.text = "④ 獲得点数：30点 (実点に引き下げ)"
	line_penalty.add_theme_font_override("font", DeskTheme.get_font())
	line_penalty.add_theme_font_size_override("font_size", DeskTheme.scaled_font(16))
	line_penalty.add_theme_color_override("font_color", Color("#ffcc02"))
	line_penalty.modulate.a = 0
	detail_vbox.add_child(line_penalty)
	
	var line_final = Label.new()
	line_final.text = "⇒ 追加の減点ペナルティは無し！"
	line_final.add_theme_font_override("font", DeskTheme.get_font())
	line_final.add_theme_font_size_override("font_size", DeskTheme.scaled_font(18))
	line_final.add_theme_color_override("font_color", Color("#ffcc02"))
	line_final.modulate.a = 0
	detail_vbox.add_child(line_final)
	
	rtb.text = "[b]嘘バレ[/b]：ダウトで嘘が暴かれると、点数は[color=#ff4081][b]実点まで引き下げられます[/b][/color]（追加のペナルティは無し）。\n[b]ダウト成功[/b]：相手の盛り幅の75%＋6点を[color=#4caf50][b]加算[/b][/color]。 / [b]失敗[/b]：15〜21点の[color=#ff4081][b]ペナルティ減点[/b][/color]。"
	...
	page.set_meta("play_animations", func():
		...
		tw.tween_property(line_actual, "modulate:a", 1.0, 0.4)
		tw.tween_property(line_declared, "modulate:a", 1.0, 0.4)
		tw.tween_interval(0.3)
		tw.tween_property(line_result, "modulate:a", 1.0, 0.4)
		tw.tween_interval(0.3)
		tw.tween_property(line_penalty, "modulate:a", 1.0, 0.4)
		tw.tween_property(line_final, "modulate:a", 1.0, 0.4)
...
func _disable_mouse_interactions(node: Node) -> void:
	if node is Control:
	...
	for child in node.get_children():
		_disable_mouse_interactions(child)

func _create_card_back() -> PanelContainer:
	var card = PanelContainer.new()
	card.custom_minimum_size = Vector2(160, 220)
	card.size = Vector2(160, 220)
	card.pivot_offset = Vector2(80, 110)
	
	var card_style = StyleBoxFlat.new()
	card_style.bg_color = DeskTheme.COLOR_CRAFT.darkened(0.12)
	...
	inner_style.border_color = Color(DeskTheme.COLOR_INK, 0.25)
