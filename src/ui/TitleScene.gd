...
func _ready() -> void:
	...
	start_btn.custom_minimum_size = Vector2(360, 70)
	start_btn.pivot_offset = Vector2(180, 35)
	start_btn.add_theme_font_override("font", load(DeskTheme.FONT_HANDWRITING))
	...
	loadout_btn = Button.new()
	loadout_btn.text = "デッキ編成"
	loadout_btn.custom_minimum_size = Vector2(160, 50)
	loadout_btn.add_theme_font_override("font", load(DeskTheme.FONT_HANDWRITING))
	loadout_btn.add_theme_font_size_override("font_size", 18)
	loadout_btn.pressed.connect(_on_loadout_pressed)
	row_hbox.add_child(loadout_btn)
	
	zukan_btn = Button.new()
	zukan_btn.text = "アイテム図鑑"
	zukan_btn.custom_minimum_size = Vector2(160, 50)
	zukan_btn.add_theme_font_override("font", load(DeskTheme.FONT_HANDWRITING))
	zukan_btn.add_theme_font_size_override("font_size", 18)
	zukan_btn.pressed.connect(_on_zukan_pressed)
	row_hbox.add_child(zukan_btn)
	
	gacha_btn = Button.new()
	gacha_btn.text = "購買部ガチャ"
	gacha_btn.custom_minimum_size = Vector2(160, 50)
	gacha_btn.add_theme_font_override("font", load(DeskTheme.FONT_HANDWRITING))
	...
	tutorial_btn.custom_minimum_size = Vector2(160, 50)
	tutorial_btn.add_theme_font_override("font", load(DeskTheme.FONT_HANDWRITING))
...
func show_mode_selection_modal() -> void:
	...
	daily_title.text = "📝 デイリー試験 (1日1回・5日制リアルタイム戦)"
	...
	var is_played_today = (Global.daily_last_played_date == today_str)
	
	var daily_desc = Label.new()
	if is_played_today:
		daily_desc.text = "今日の自習は完了しました。次の日は明日プレイ可能です。(" + today_str + ")"
		daily_desc.add_theme_color_override("font_color", Color("d32f2f"))
		daily_btn.disabled = true
	else:
		daily_desc.text = "毎日1日分(3時限)プレイし、5日間でマッチ完結。日付固定デッキで全国のゴーストと対戦！"
	...
	daily_btn.pressed.connect(func():
		...
		var start_daily_match = func(ghosts: Array):
			...
			Global.daily_opponent_ghosts[str(day_idx)] = ghosts
			Global.save_game()
...
func _on_tutorial_next_pressed() -> void:
	...
	if current_tutorial_page < 5:
		...
		tween.tween_property(tutorial_modal, "scale", Vector2.ZERO, 0.25)
		tween.chain().tween_callback(func():
